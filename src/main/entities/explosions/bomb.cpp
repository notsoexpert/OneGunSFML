...
void Create(Setup& setup) {
    ...
    //SetupCollidable(setup, CollisionRect); // Currently only boom wave does damage (see below)
...
void CreateBoomWave(entt::registry &registry, entt::entity boomTicker) {
    ...
    auto& waveRenderable = registry.emplace<Renderable>(waveEntity, GetTexture(Images::ExplosionYellow));
    ...
    registry.emplace<Rotating>(waveEntity, sf::radians(4*Pi));
    ...
    registry.emplace<Component>(waveEntity, Type::Bomb, WaveDamageFactor);
    registry.emplace<Lifetime>(waveEntity, sf::seconds(WaveLifeTimeInSeconds), 
